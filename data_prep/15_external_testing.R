...
#for (i in 1:length(methods)){
  ...
  eg <- grid.arrange(plots[[1]], plots[[2]], plots[[3]], nrow = 2,
                     top=textGrob(paste("external evaluation final", method, "model, trained on 150000 observations"),
  ...
  lapply(seq(hi), function(k){
      ...
      writeRaster(sp, paste0(predoutpath, "pred_", prednam, ".tif"), overwrite=T)
  })
...
#}
  ...
  pat <- "2019006.2005"
  pos_pot <- which(grepl(pat, nam_rasslice))
  
  sp <- raster(list.files(predoutpath, pattern=pat, full.names=T))
  plotstack <-stack(rasslice[[pos_pot]][[1]], sp, rasslice_landsat[[pos_pot]][[1]])
  names(plotstack) <- c(paste0("MODIS", pat), "prediction", "Landsat")
  # tmp <- spplot(plotstack)
  # png(filename=paste0(predoutpath, "pngs/plot_", pat, ".png"))
  # print(tmp)
  # dev.off()
