...
for (i in 1:length(methods)){
  ...
  fm <- list.files(modelpath, full.names=T, pattern=method)
  fm <- fm[grepl("dummys", fm)]
  fm <- fm[grepl("ffs", fm)]
  print(fm)
  ...
  if(method=="nnet"){ # importance = TRUE kills it
    ...
    # predictors$landcoverres.1 <- factor(predictors$landcoverres.1)
    model_final <- train(predictors,
                         response, 
                         method=method, 
                         trControl=tctrl,
  ...
  save(model_final,file=paste0(modelpath,"final_model_",method,"_", n, "dummyrun.RData"))
}
