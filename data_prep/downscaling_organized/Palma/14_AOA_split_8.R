...
# read predstack
predstack <- stack(list.files(predstackdir, pattern="split_8.tif", full.names = T))
...
modname <- basename(list.files(predstackdir, pattern="split_8.tif", full.names = T))
# TeAqNum <- predstack[[1]]
...
saveRDS(aoa_spat, paste0(aoapath, "aoa_spatial_", method, "_", "split_8.RDS"))
...
writeRaster(aoa_ras, paste0(aoapath, "aoa_spatial_", method, "_", "split_8.tif"),
            overwrite=T)
