...
outpath <- "/scratch/tmp/llezamav/modeling_after_paper/"
trainpath <- "/scratch/tmp/llezamav/train_test_after_paper/train_valid/"
# trainpath <- "D:/LST_after_paper/train_valid/"
...
train <- read.csv2(paste0(trainpath, "train_LHS_50000_scaled.csv"))
...
train$soilraster_fac <- factor(train$soilraster)
train$TeAq_fac <- factor(train$TeAq)
train$landcover_fac <- factor(train$landcoverres)

predictors <- train[,c("Modis_sc","ia_sc", "hs_sc", "dem_sc", 
                       "slope_sc", "aspect_sc", "TWI_sc", 
                       "TeAq_fac", "soilraster_fac", "landcover_fac")]
length(predictors)

cores <- detectCores()
print(paste("cores = ", cores))

cl <- makeCluster(cores-3, outfile="/home/l/llezamav/LST_after_paper/par_algorithms.txt")
