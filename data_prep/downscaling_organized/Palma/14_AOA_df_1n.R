...
# # that's where stuff should be printed to
# opts_slurmR$get_tmp_path()
# 
# cl <- makeSlurmCluster(n=90, # equals --ntasks 
#                        job_name = "AOA_df_slurmR",
#                        max_wait = 14400*6, # waiting 24 hours max for batch job to start
#                        mem = "160G",
#                        time=160, # in hours
#                        tmp_path = opts_slurmR$get_tmp_path(),
#                        cluster_opt = list(nnodes=3, # those stem from makePSOCKCluster
#                                           outfile="/home/l/llezamav/scripts_new/aoa_slurmR.txt"))
# try(opts_slurmR$get_opts_job(), silent = T)
# 
# 
# jnk = clusterEvalQ(cl,{library(raster); 
#   library(rgdal); library(CAST,lib.loc="/home/l/llezamav/RCast/"); 
#   library(raster)})
# clusterExport(cl, list("aoapath", "method", "model_final", "predstack_df",
#                        "predstack_df_within_ex", "predstack", "withinextent"))
