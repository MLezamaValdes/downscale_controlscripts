...
outputpath <- paste0(datpath, "train_test_DI/")
...
di_log_choosing <- function(extr, ym){
    ...
    # } else {
      #cli <- makeCluster(27, outfile="/home/l/llezamav/par_aoa_out.txt")
      cli <- makeCluster(2, outfile="/home/l/llezamav/par_aoa_out.txt")
...
parLapply(cl, seq(ex_files), function(m){
      ...
      extr <- read.csv2(ex_files[m])
      di_log_choosing(extr, ym)
})
