#https://rdrr.io/github/16EAGLE/getSpatialData/src/R/internal.R
...
# had to use "map <- leafem::addFeatures(map, aoi.sf)" instead of "map <- addFeatures(map, aoi.sf)"
...
#' @noRd
library(leafem)
.EE_preview_mod <- function(record, preview_crs = NULL, on_map = TRUE, show_aoi = TRUE, verbose = TRUE){
  if(inherits(verbose, "logical")) options(gSD.verbose = verbose)
    ...
    if(isTRUE(on_map)){
      ...
      if(isTRUE(show_aoi)){
        if(isFALSE(getOption("gSD.aoi_set"))){
          ...
          aoi.sf <- getOption("gSD.aoi")
          #aoi.sf <- .make_aoi(aoi.m, type = "sf", quiet = T)
          map <- leafem::addFeatures(map, aoi.sf)
        }
      }
      map # display mapview or leaflet output
    } else{
...
getLandsat_preview_mod  <- function(record, on_map = TRUE, show_aoi = TRUE, verbose = TRUE){
  
  .EE_preview_mod(record = record, on_map = on_map, show_aoi = show_aoi, verbose = verbose)
}